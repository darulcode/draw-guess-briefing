(function () {
  ...
  function resultHtml(state, projector = false) {
    ...
    return `<div class="center"><span class="pill ${skipped ? 'round_result' : 'drawing'}">Ronde ${result.roundNumber} ${skipped ? 'dilewati' : 'selesai'}</span><h2 style="margin:18px 0 8px">${skipped ? 'Poin ronde dibatalkan' : 'Kata yang benar'}</h2><div class="result-word">${escapeHtml(result.word)}</div></div><div class="grid two" style="margin-top:28px;text-align:left"><div><h3>Urutan jawaban benar</h3>${correctRows}</div><div><h3>Bonus penggambar</h3><div class="metric"><span>${escapeHtml(result.drawer?.name || '-')}</span><strong>+${result.drawer?.bonus || 0} poin</strong></div><h3 style="margin-top:20px">Leaderboard sementara</h3>${leaderboardHtml(state.leaderboard, projector ? 8 : undefined)}</div></div>`;
  }
